...
class Object {
	...
	std::vector<Complex> points;	// vertices of the object
public:
	Object() : points({ Complex(0, 0), Complex(-1, -1), Complex(0, 1), Complex(1, -1) }) {
	...
	void Animate(float t) {
		...
		std::vector<Complex> transPoints = points;
		for (auto& point : transPoints) {
			point = ((point - Complex(1, -1)) * Polar(2, t) + Complex(1, -1) + Complex(2, 3)) * Polar(0.8f, -t / 2);
		}
	...
	void Draw() {
		...
		gpuProgram.setUniform(vec3(1, 0, 1), "color");
